<template>
    <div
        ...
        :class="{show: isActive({path: parentSlug, children: items})}"
    >
        ...
        methods: {
            toggleWithChildrenHandling(path) {
                this.items.filter(i => i.path.startsWith(path))
                    .forEach(i => {
                        if (this.isActive(i) || i.path === path) {
            ...
            isActive(item) {
                ...
                if (this.activeSlug.startsWith(item.path)) {
                    return !this.toggled.includes(item.path);
                }
            ...
            getClass(item, depthLevel, disabled) {
                const isActiveItem = this.isActive(item);

                return {
                    bold: depthLevel === 1,
                    section: item.isSection,
                    active: isActiveItem,
                    disabled: isActiveItem && disabled
                }
            }
        }
    }
