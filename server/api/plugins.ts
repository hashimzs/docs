...
import type {JSONProperty, JSONSchema} from "@kestra-io/ui-libs";
import {type Plugin, subGroupName} from "~/utils/plugins";
import {isEntryAPluginElementPredicate} from "~/utils/plugins";
...
function nuxtBlocksFromJsonSchema(jsonSchema: JSONSchema, pageType: PageType) {
    return {
        body: {
            jsonSchema,
            toc: {
...
function nuxtBlocksFromSubGroupsWrappers(subGroupsWrappers: Plugin[], pageType: PageType) {
    return {
        body: {
    ...
    try {
        ...
        switch (type) {
            ...
            case "definitions": {
                ...
                return nuxtBlocksFromJsonSchema(pageData.schema, type);
            }
            case "plugin": {
                let subgroups = await $fetch(`${config.public.apiUrl}/plugins/${page}/subgroups`);

                return nuxtBlocksFromSubGroupsWrappers(subgroups, type);
            }
            case "navigation": {
